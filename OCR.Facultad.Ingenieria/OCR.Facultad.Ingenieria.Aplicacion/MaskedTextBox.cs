...
//				m_mask = Masked;
			...
			switch(m_mask)
			{
				case Mask.DateOnly:
					...
					if(!regStr.IsMatch(sd.Text))
						errorProvider1.SetError(this, "Fecha no válida; mm/dd/yyyy");
					break;
				case Mask.PhoneWithArea:
					regStr = new Regex(@"\d{3}-\d{3}-\d{4}");
					if(!regStr.IsMatch(sd.Text))
						errorProvider1.SetError(this, "Número de teléfono no válido; xxx-xxx-xxxx");
					break;
				case Mask.IpAddress:
                    if (this.Text != "localhost")
                    {
                        short cnt = 0;
                        ...
                        for (short i = 0; i < len; i++)
                            if (sd.Text[i] == '.')
                            {
                                ...
                                if (i + 1 < len)
                                    if (sd.Text[i + 1] == '.')
                                    {
                                        errorProvider1.SetError(this, "La dirección IP, no es válida.");
                                        break;
                                    }
                            }
		...
		private void MaskDigit(KeyPressEventArgs e)
		{
			...
			{
				e.Handled = true;
				errorProvider1.SetError(this, "Unicamente válido para un dígito.");
			}
		}
        ...
        private void MaskIpAddr(KeyPressEventArgs e)
        {
            ...
            errorProvider1.SetError(this, "");
            string val = Convert.ToString(e.KeyChar);
            bool eslocalhost = false;
            //string temp=
            if (this.Text.Length == 0)
            {
                if (valor[0]==e.KeyChar)
                {
                    eslocalhost = true;
                }
            }
            else
            {
                if (e.KeyChar != 8)
                {
                    string cad = this.Text + val;
                    if (valor.Contains(cad))
                    {
                        if (cad.Length <= valor.Length)
                        {
                            eslocalhost = true;
                    ...
                    if (cad.Length  >valor.Length )
                    {
                        string sub = cad.Replace(valor, "");
                        if (sub.Length != cad.Length)
                        {
                           // string v = e.KeyChar.ToString();
                            e.Handled = true;
                           // this.Text = valor;
                            eslocalhost = true;
                        }
            ...
            if (!eslocalhost)
            {
                ...
                { // only digit, Backspace and dot are accepted
                    ...
                    if (e.KeyChar != 8)
                    {
                        if (e.KeyChar != '.')
                        {
                            ...
                            if (digitPos == 3)
                            {
                                ...
                                if (Int32.Parse(tmp2) > 255) // check validation
                                    errorProvider1.SetError(this, "El número no puede ser mayor que 255");
                                else
                                {
                                    if (DelimitNumber < 3)
                                    {
                        ...
                        {   // added - MAC
                            ...
                            {
                                errorProvider1.SetError(this, "No mas . por favor!");
                            ...
                            {	// got the right number, but don't allow two in a row
                                ...
                                {
                                    errorProvider1.SetError(this, "No pueden haber dos puntos seguidos.");
                                    e.Handled = true;
                                }
                                else
                                {	// ok, add the dot
                ...
                {
                    e.Handled = true;
                    errorProvider1.SetError(this, "Ruta no válida");
                }
